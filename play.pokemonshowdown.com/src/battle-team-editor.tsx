...
export type InnerFocusType = 'pokemon' | 'ability' | 'item' | 'move' | 'stats' | 'details' | 'import';
...
class TeamEditorForm extends preact.Component<{
	...
	renderInnerFocus() {
		...
		return <div class="team-focus-editor" onKeyDown={editor.handleParentKeyDown}>
			<div class={isSearchMode && (set?.moves.length || 0) > 5 ? 'team-focus-top' : ''}>
				<ul class="tabbar">
					<li class="home-li"><button
						class="button" onClick={editor.setEditor?.back || this.closeInnerFocus}
						title={editor.setEditor?.titles?.back}
					>
			...
			{type === 'stats' ? (
				editor.setEditor?.render?.stats ? editor.setEditor.render.stats(editor, setIndex) :
				<StatForm editor={editor} set={set!} onChange={this.handleSetChange} />
			) : type === 'details' ? (
				editor.setEditor?.render?.details ? editor.setEditor.render.details(editor, setIndex) :
				...
				<PSSearchResults
					...
					onSelect={this.selectResult}
				>
					{type === 'ability' && !editor.setEditor?.hideSampleSets && <SetSourceButtons
	...
	renderInput(
		...
		return <input
			...
			title={editor.setEditor?.titles?.[type]}
	...
	renderSet(set: Dex.PokemonSet | undefined, i: number) {
		...
		if (!set) {
			return <div class="set-form" data-set-index={i}>
				<div style="text-align:right">
					{setEditor?.render?.emptyActions ? setEditor.render.emptyActions(editor, i) : <>
						{editor.deletedSet ? (
		...
		return <div class={`set-form${isCur}`} data-set-index={i}>
			<div style="text-align:right">
				{setEditor?.render?.actions?.(editor, i)}
				{!setEditor?.hideCopy && <button
					class="option" onClick={this.copySet} value={i} title={setEditor?.titles?.copy}
				>
				...
				{!(TeamEditorState.clipboard || editor.readonly) && <button
					class="option" name="import" onClick={this.clickPanelButton}
					value={`set-${i}-import`} title={setEditor?.titles?.import}
				>
					<i class="fa fa-upload" aria-hidden></i> {TL`[Import/Export]`}
				</button>} {}
				{!(TeamEditorState.clipboard || editor.readonly) && <button
					class="option" name="delete" onClick={setEditor?.deleteSet || this.deleteSet} value={i}
					title={setEditor?.titles?.delete}
				>
					<i class="fa fa-trash" aria-hidden></i> {TL`[Delete]`}
				</button>}
			</div>
			<table class={`${spriteClass}${tintClass}`} style={sprite}>
				<tr>
					<td rowSpan={2} class="set-pokemon"><div class="border-collapse">
						<span class="sprite-inner">
							<label class="label">
								<span>{TL.term.pokemon}</span> {setEditor?.render?.rename?.(editor, i)}
					...
					<td colSpan={2} class="set-details"><div class="border-collapse">
						<label class="label">
							...
							>
								{setEditor?.render?.detailCell ? setEditor.render.detailCell(editor, i) : <>
									<span class="detailcell">
						...
						{setEditor?.render?.types ? setEditor.render.types(editor, i) : <div>
							{species.types.map(type => <><PSIcon type={type} new={!editor.narrow} /> </>)}
						</div>}
					</div></td>
					<td rowSpan={2} class={`set-moves${overfull}`}>{setEditor?.render?.moves ?
						setEditor.render.moves(editor, i) : <div class="border-collapse">
					...
					<td rowSpan={2} class="set-stats">
						<label class="label">
							...
							>
								{setEditor?.render?.statCell ? setEditor.render.statCell(editor, i) :
				...
				<tr>
					{setEditor?.render?.abilities ? setEditor.render.abilities(editor, i) : <>
			...
			<div class={`set-nickname${tintClass}`}>
				{setEditor?.render?.nickname ? setEditor.render.nickname(editor, i) : <label class="label">
...
export class SetImportForm extends preact.Component<{
	...
	getExportText() {
		...
		if (editor.setEditor?.importExport) return editor.setEditor.importExport.export(editor, setIndex);
		if (!this.props.set) return '';
		return Teams.exportSet(this.props.set, editor.dex).trim();
	}
	override componentDidMount() {
		this.setRevertPoint();
	}
	...
	revertSetToRevertPoint() {
		const { editor, setIndex } = this.props;
		if (editor.setEditor?.importExport) {
			editor.setEditor.importExport.import(editor, setIndex, this.revertText);
			this.props.onChange();
			return true;
		}
	...
	inputText = () => {
		...
		if (editor.setEditor?.importExport) {
			const error = editor.setEditor.importExport.import(editor, setIndex, this.textbox.value);
	...
	override render() {
		...
		return <div role="dialog" aria-label={TL`Import/Export`} class="set-import-form">
			<div class="resultheader"><h3>
				{editor.setEditor?.importExport?.label || TL`Import/Export set`}
			</h3></div>
			<div class="pad">
				<p>
				...
				<textarea
					...
					onInput={editor.setEditor?.importExport ? undefined : this.inputText}
					onChange={editor.setEditor?.importExport ? this.inputText : undefined}
					style="min-height:6em"
				></textarea>
				{!editor.setEditor?.hideSampleSets && <SetSourceButtons
